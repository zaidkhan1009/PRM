...
public class ProductSaleTestCases extends BaseClass {
	...
	/*
	 * Validating all the Field and Button on Product Listing and validating Details
	 * show on individual product
	 */
...
//        Assert.assertTrue ( ProductSalePageActions.checkSearchProduct () );
	...
	/*
	 * Validating the Error Msg shown on Product Listing when mandatory field are
	 * not filled and user click on save button
	 */
	...
	/*
	 * Verify the redirection on click of cancel button and checking on reset field
	 * are getting blank
	 */
	...
	/*
	 * Verify product search by name and batch on Product listing and matching the
	 * result according to the search
	 */
	...
	public void verifyProductSearch() {
		...
		Assert.assertTrue(ProductSalePageActions
				.verifyProductSearchByName(productSaleTestData.get("product_Name").split(" ")[0]));
		ProductSalePageActions.enterSearchedDetails(productSaleTestData.get("product_BatchNumber").substring(0,
				productSaleTestData.get("product_BatchNumber").length() - 3));
		Assert.assertTrue(ProductSalePageActions.verifyProductSearchByBatchNumber(productSaleTestData
				.get("product_BatchNumber").substring(0, productSaleTestData.get("product_BatchNumber").length() - 3)));
	...
	public void verifyProductsAndDiscountOnProductListing() {
		Assert.assertTrue(ProductSalePageActions.verifyProductListingData());
		Assert.assertEquals(ProductSalePageActions.verifyProductCompanyName(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), productSaleTestData.get("product_Company"));
		Assert.assertTrue(ProductSalePageActions.verifyProductName(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")));
		Assert.assertTrue(ProductSalePageActions.verifyProductBatchNumber(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")));
		Assert.assertEquals(ProductSalePageActions.verifyProductSize(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), productSaleTestData.get("product_Size"));
		Assert.assertTrue(ProductSalePageActions.verifyProductOriginalPrice(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), productSaleTestData.get("product_Original_Price")));
		Assert.assertTrue(ProductSalePageActions.verifyProductPriceAfterDiscount(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber"), 5));
		Assert.assertTrue(ProductSalePageActions.verifyQtyDiscountTagOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 1, 5));
		ProductSalePageActions.addProductQtyToCart(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 1);
		Assert.assertTrue(ProductSalePageActions.verifyQtyDiscountTagOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 2, 15));
		ProductSalePageActions.addProductQtyToCart(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 1);
		Assert.assertTrue(ProductSalePageActions.verifyProductOriginalPrice(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), productSaleTestData.get("product_Original_Price")));
		Assert.assertTrue(ProductSalePageActions.verifyProductPriceAfterDiscount(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber"), 15));
		Assert.assertTrue(ProductSalePageActions.verifyToolTipOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")));
		Assert.assertTrue(ProductSalePageActions.verifySpecialDiscountTagOnProduct(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber"),
				productSaleTestData.get("discount_From_Date"), productSaleTestData.get("discount_To_Date")));
		ProductSalePageActions.clickOnEditProductTab();
		ProductSalePageActions.clickOnDeleteButton(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"));

	}

	/*
	 * Verify No Product Found error msg found when there is no Product present in
	 * clinic
	 */
	@Test
	...
	/*
	 * Verifying adding product to cart and removing some qty of added item Verify
	 * NetAmount,Unit COst,Coupon Value and Amt.Payables values removing qty from
	 * product listing tab and then checking qty is also remove from cart
	 */
	@Test(description = VERIFY_ADD_REMOVE_ITEM_ON_PRODUCT_LISTING)
	public void verifyAddRemoveProductOnListing() {
		ProductSalePageActions.addProductQtyToCart(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 2);
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), 2);
		...
		ProductSalePageActions.removeProductQtyFromCart(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 2);
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), 0);
		...
		ProductSalePageActions.addProductQtyToCart(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 3);
		Assert.assertEquals(ProductSalePageActions.getNumberOfProductInCart(), 1);
		ProductSalePageActions.addProductQtyToCart(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"), 2);
		...
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), 0);
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber")), 2);
		Assert.assertEquals(ProductSalePageActions.getNumberOfProductInCart(), 1);
		Assert.assertTrue(ProductSalePageActions.verifyAmtPayableOfProductAddedInCart());
		...
		Assert.assertFalse(ProductSalePageActions.checkAddedProductNameDisplayed(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")));
		Assert.assertFalse(ProductSalePageActions.checkAddedProductBatchDisplayed(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")));

		Assert.assertTrue(ProductSalePageActions.checkAddedProductNameDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		Assert.assertTrue(ProductSalePageActions.checkAddedProductBatchDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		...
		Assert.assertTrue(ProductSalePageActions.checkAddedProductNameDisplayed(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")));
		Assert.assertTrue(ProductSalePageActions.checkAddedProductBatchDisplayed(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")));
		Assert.assertEquals(
				ProductSalePageActions.checkAddedProductCompanyShortNameDisplayed(
						productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")),
				productSaleTestData.get("product_Company"));
		Assert.assertTrue(ProductSalePageActions.checkAddedProductNameDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		Assert.assertTrue(ProductSalePageActions.checkAddedProductBatchDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		Assert.assertEquals(
				ProductSalePageActions.checkAddedProductCompanyShortNameDisplayed(
						productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")),
				productSalesTestData1.get("product_Company"));

		ProductSalePageActions.clickOnProductSaleTab();
		ProductSalePageActions.addProductQtyToCart(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"), 1);
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber")), 3);
		Assert.assertTrue(ProductSalePageActions.verifyAmtPayableOfProductAddedInCart());

		ProductSalePageActions.clickOnEditProductTab();
		Assert.assertTrue(ProductSalePageActions.verifyDetailsOfProductInCart());
		Assert.assertTrue(ProductSalePageActions.verifyTotalAmountPayableOfCart());
		Assert.assertEquals(ProductSalePageActions.getAddedQtyOfProductOnListing(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")), 3);
		ProductSalePageActions.clickOnDeleteButton(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"));
		ProductSalePageActions.clickOnDeleteButton(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"));

	}

	/*
	 * Increasing/Decreasing the item qty from cart tab and verifying Amounts values
	 * and then checking in Product Listing Page Deleting added product from Cart
	 * and verifying Amount values after item remove from cart.
	 */
	...
	public void verifyAddRemoveProductOnCartTab() {
		...
		Assert.assertTrue(ProductSalePageActions.verifyTotalAmountPayableOfCart());
		Assert.assertEquals(ProductSalePageActions.getAddedQtyOfProductOnListing(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")), 3);

		ProductSalePageActions.increaseQtyOfAddedProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 2);
		Assert.assertEquals(ProductSalePageActions.getAddedQtyOfProductOnListing(
				productSaleTestData.get("product_Name"), productSaleTestData.get("product_BatchNumber")), 5);
		Assert.assertTrue(ProductSalePageActions.verifyDetailsOfProductInCart());
		...
		ProductSalePageActions.clickOnProductSaleTab();
		Assert.assertEquals(ProductSalePageActions.getAddedQtyCountOnProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber")), 5);
		Assert.assertTrue(ProductSalePageActions.verifyAmtPayableOfProductAddedInCart());
		...
		Assert.assertTrue(ProductSalePageActions.checkAddedProductNameDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		Assert.assertEquals(ProductSalePageActions.getAddedQtyOfProductOnListing(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")), 2);

		ProductSalePageActions.decreaseQtyOfAddedProduct(productSaleTestData.get("product_Name"),
				productSaleTestData.get("product_BatchNumber"), 2);
		Assert.assertTrue(ProductSalePageActions.checkAddedProductNameDisplayed(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		Assert.assertEquals(ProductSalePageActions.getAddedQtyOfProductOnListing(
		...
		ProductSalePageActions.clickOnEditProductTab();
		ProductSalePageActions.decreaseQtyOfAddedProduct(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"), 2);
		...
		ProductSalePageActions.clickOnProductSaleTab();
		Assert.assertTrue(ProductSalePageActions.verifyAmtPayableOfProductAddedInCart());
		Assert.assertFalse(ProductSalePageActions.verifyProductSelectedOnListing(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
	...
	/*
	 * Creating New UnApproved patient from PRM and then checking patient is created
	 * by searching patient in the clinic Verifying UnApproved patient is created
	 * and checking Patient communication.
	 */
...
//        InvoiceListingPageActions.actionsButton_InvoicesCreatedDate();
	...
	/*
	 * Verifying Product Sale for existing Patient Verifying if patient is not
	 * present in clinic from where he/she is buying product then after product sale
	 * patient is getting mapped to that clinic or not Verifying if patient do not
	 * have email and user enter email from product sales>>Getting updated in
	 * patient profile.
	 */
...
//        InvoiceListingPageActions.actionsButton_InvoicesCreatedDate();
	...
	/*
	 * Checking the Auto Suggestion when existing patient Name/Mobile Number/Patient
	 * Id is entered.
	 */
	...
	/*
	 * Verify when user have added some item to cart and user try to exit screen
	 * without creating the invoice of product added in cart
	 */
	@Test(priority = 1)
...
//        Assert.assertEquals ( CommonPageActions.verification (),"Clove Dental :: Login Access To Clinical View" );
	...
	/*
	 * Verify Product with Expiry Near to Configured days is removed on product
	 * listing Verify Product with Expiry Near to Configured days are Highlighted
	 */
	...
	/*
	 * Verify Error Msg shown when No Qty of item is added and user click on minus
	 * button Verify Error Msg shown when user has already the max available qty.
	 */
	@Test
	public void verifyMinMaxQtyErrorOnProductListing() {
		ProductSalePageActions.removeProductQtyFromCart(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"), 1);
		Assert.assertTrue(ProductSalePageActions.verifyZeroQuantityErrorMsg());
		ProductSalePageActions.addProductQtyToCart(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"),
				Integer.valueOf(productSalesTestData1.get("available_Qty")));
		Assert.assertTrue(ProductSalePageActions.verifyMaxQtyAddedErrorMsg(productSalesTestData1.get("available_Qty")));
		Assert.assertTrue(ProductSalePageActions.verifyPlusIconOnAddingMaxQtyOnListing(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		ProductSalePageActions.clickOnEditProductTab();
		ProductSalePageActions.increaseQtyOfAddedProduct(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"), 1);
		Assert.assertTrue(ProductSalePageActions.verifyMaxQtyAddedErrorMsg(productSalesTestData1.get("available_Qty")));
		Assert.assertTrue(ProductSalePageActions.verifyPlusIconOnAddingMaxQtyOnCart(
				productSalesTestData1.get("product_Name"), productSalesTestData1.get("product_BatchNumber")));
		ProductSalePageActions.clickOnDeleteButton(productSalesTestData1.get("product_Name"),
				productSalesTestData1.get("product_BatchNumber"));
	}
	...
	/*
	 * Verifying Product Sales Button Within Patient Dashboard and Redirection to
	 * Product Sale on click
	 */
	@Test
