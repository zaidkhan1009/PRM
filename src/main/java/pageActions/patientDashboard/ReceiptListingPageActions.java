...
public class ReceiptListingPageActions extends BaseClass {
	...
	public static void checkData() {
		BaseClass.waitForPageLoad();
		Assert.assertTrue(checkedWebElementDisplayed(receiptListingPage.getSendReceiptBtn())
				&& checkedWebElementDisplayed(receiptListingPage.getSearchBtn())
				&& checkedWebElementDisplayed(receiptListingPage.getShowDetailsBtn())
				&& checkedWebElementDisplayed(receiptListingPage.getFromDateField())
				&& receiptListingPage.getFromDateField().getAttribute("placeholder").contains("» from (dd-mm-yy)")
				&& checkedWebElementDisplayed(receiptListingPage.getDueAmount())
				&& checkedWebElementDisplayed(receiptListingPage.getToDateField())
				&& receiptListingPage.getToDateField().getAttribute("placeholder").contains("» to (dd-mm-yy)"));
	}

	public static void checkReceiptListingTableHead() {
		BaseClass.waitForSpinnerToDisappear();
		BaseClass.waitForElementVisibility(receiptListingPage.getDateHead(), 4000);
		Assert.assertTrue(checkedWebElementDisplayed(receiptListingPage.getDateHead())
				&& checkedWebElementDisplayed(receiptListingPage.getReceiptHead())
				&& checkedWebElementDisplayed(receiptListingPage.getInvoiceHead())
				&& checkedWebElementDisplayed(receiptListingPage.getActionHead())
				&& checkedWebElementDisplayed(receiptListingPage.getAmountPaidHead())
				&& checkedWebElementDisplayed(receiptListingPage.getPaymentMethodHead())
				&& checkedWebElementDisplayed(receiptListingPage.getDoctorHead()));
	}
	...
	public static long checkAdvanceAmount() {
		...
		try {
			amountAdvance = NumberFormat.getNumberInstance(Locale.US)
					.parse(receiptListingPage.getAdvanceAmount().getText()).intValue();
	...
	private long checkDueAmount() {
		...
		long amountDue = 0;
		try {
			amountDue = NumberFormat.getNumberInstance(Locale.US).parse(receiptListingPage.getDueAmount().getText())
					.intValue();
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static void receiptDataTable() {
		...
		for (WebElement row : receiptListingPage.getRowTreatmentColumn()) {
			...
			if (paymentMethod.contains("-")) {
				...
				Assert.assertTrue(!(paymentMode[0].contains("NA") || paymentMode[0].contains("null"))
						&& !(paymentMode[1].contains("NA") || paymentMode[1].contains("null")));
			} else {
			...
			// cells.get(5).findElement(By.xpath("//span[@class='dis-block']/../following-sibling::td[3]"));
			// coupon amount of receipt
			WebElement ptrIdStatus = cells.get(5)
					.findElement(By.xpath("//span[@class='dis-block']/../following-sibling::td[4]"));
			...
			boolean flag2 = !(invoiceNumber.contains("null") || invoiceNumber.contains("NA")
...
//			Assert.assertTrue(flag1 && flag2   && flag6  && flag8);
	...
	// checking the actions button of all the invoices on the basis of invoices
	// created date
	public static void actionsButton_ReceiptsCreatedDate() {
		...
		for (WebElement row : receiptListingPage.getRowTreatmentColumn()) {
			...
			// checking if invoice created date is within 24 hr hour but not on same day
			// then delete,print,receipt,view only appear
			if ((Math.abs(date.getTime() - receiptCreatedDate.getTime()) < MILLIS_PER_DAY)) {
				Assert.assertTrue(checkedWebElementDisplayed(viewBtn) && checkedWebElementDisplayed(printBtn)
						&& checkedWebElementDisplayed(editBtn) && checkedWebElementDisplayed(deleteBtn));
	...
	public static void openViewModal() {
		...
		if (receiptListingPage.getRowTreatmentColumn().size() > 0) {
			...
			if (receiptListingPage.getRowTreatmentColumn().size() > 0) { // 6
				...
				WebElement viewBtn = cells.get(7)
						.findElement(By.xpath("//a[contains(@class,'patientReceiptViewCall')]"));
				viewBtn.click();
			} else {
...
//		driver.navigate().refresh();
	...
	public static void checkDataInView() {
		BaseClass.waitForPageLoad();
		Assert.assertTrue(checkedWebElementDisplayed(receiptListingPage.getInfoHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getReceiptNoHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getReceiptDateHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getPaymentDetailsHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getAmountReceivedHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getModePaymentHeadInView())
				&& checkedWebElementDisplayed(receiptListingPage.getReceivedAsHeadInView())
	...
	public static void viewData() {
		...
		String receiptListingDAte = driver.findElement(By.xpath("//tbody[@id='" + receipt + "']/tr/td[2]")).getText()
				.trim();
		String receiptListing_receipt = driver.findElement(By.xpath("//tbody[@id='" + receipt + "']/tr/td[3]"))
				.getText().trim();
		String receiptListing_AmountPaid = driver.findElement(By.xpath("//tbody[@id='" + receipt + "']/tr/td[5]"))
				.getText().trim();
		String receiptListing_modePayment = driver.findElement(By.xpath("//tbody[@id='" + receipt + "']/tr/td[6]"))
				.getText().trim();
		String receiptListing_Doctor = driver.findElement(By.xpath("//tbody[@id='" + receipt + "']/tr/td[7]")).getText()
				.trim();
		...
		boolean note_flag = !(receiptListingPage.getNotes().getText().contains("NA")
				|| receiptListingPage.getNotes().getText().contains("null"));
...
//		Assert.assertTrue(receipt_flag&&ReceiptDate_flag&&amount_flag&&modePayment_flag&&note_flag&&receivedAs_flag&&doctor_flag);
	...
	public static void clickDeleteBtn(String mode) {
		...
		WebElement ele = driver.findElement(
				By.xpath("//td[contains(text(),'" + mode + "')]/following-sibling::td/a[@class='deleteReciept btn']"));
		ele.click();
	}
	...
	public static void checkRequireDropDwn() {
		...
		String[] exp = { "Select a reason", "Advance refund", "Change in treatment plan", "Incorrect payment received",
				"Incorrect treatment plan", "Payment refund", "Others" };
